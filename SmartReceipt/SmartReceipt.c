...
* $Revision: 6185 $ (v4.040.05.011646)
...
static uint32_t dwDataLen = sizeof(baSnepAppBuf);
	    ...
	    static uint8_t ndef_record[500]={0xC1,0x01,0x00,0x00,0x01,0xED,0x54,0x02};
...
int main (void)
{
    ...
    do
    {
        ...
        phOsal_ThreadCreate(&RdLib.ThreadHandle, &RdLib, &TReaderLibrary, &sDiscLoop);
...
void TSNEPClient(void * pLlcp)
{
    ...
    { 
        ...
        do{
        sleep(1);
	    memset(ndef_record+8,0x00,492);
		printf ("\n\n@@@ write data to send @@@ \n\n");
		fflush(stdin);
	do{	printf(".");
		scanf ("%[^\n]s\n",(ndef_record+8));
				getchar();
		}while(ndef_record[8]==0x00&&ndef_record[9]==0x00&&ndef_record[10]==0x00);
		dwDataLen = sizeof(ndef_record);
		sleep(1);
		printf("\n\n==Ready to TAG!!!!==\n\n");
            ...
            if (status == PH_ERR_SUCCESS)
            {
                ...
                status = phnpSnep_Put(&snpSnepClient, (uint8_t *)ndef_record, dwDataLen);
...
static phStatus_t LoadDiscoveryConfiguration()
{
    ...
    status = phacDiscLoop_SetConfig(&sDiscLoop, PHAC_DISCLOOP_CONFIG_PAS_POLL_TECH_CFG, wPasPollConfig);
    CHECK_STATUS(status);

    /* Active poll bitmap configuration. */
    status = phacDiscLoop_SetConfig(&sDiscLoop, PHAC_DISCLOOP_CONFIG_ACT_POLL_TECH_CFG, wActPollConfig);
    CHECK_STATUS(status);

    /* Passive listen bitmap configuration. */
    status = phacDiscLoop_SetConfig(&sDiscLoop, PHAC_DISCLOOP_CONFIG_PAS_LIS_TECH_CFG, wPasLisConfig);
    CHECK_STATUS(status);

    /* Active listen bitmap configuration. */
    status = phacDiscLoop_SetConfig(&sDiscLoop, PHAC_DISCLOOP_CONFIG_ACT_LIS_TECH_CFG, wActLisConfig);
    CHECK_STATUS(status);
...
phStatus_t ReadNdefMessage(uint8_t TopTagType)
{
    ...
    status = phalTop_SetConfig(&salTop, PHAL_TOP_CONFIG_TAG_TYPE, TopTagType);
    DEBUG_ERROR_PRINT(status);

    /* Check for NDEF presence */
    status = phalTop_CheckNdef(&salTop, &bTagState);
    DEBUG_ERROR_PRINT(status);

    if((bTagState == PHAL_TOP_STATE_READONLY) || (bTagState == PHAL_TOP_STATE_READWRITE))
    {
        /* Read NDEF message */
        status = phalTop_ReadNdef(&salTop, baSnepAppBuffer, &wDataLength);
        DEBUG_ERROR_PRINT(status);
